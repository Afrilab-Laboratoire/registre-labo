...
def auto_fill_epi_expiration(df):
    """Pour les EPI à cycle fixe (6 mois), calcule automatiquement la date prochaine
    récupération à partir de la date de récupération, sauf si aucune date de récupération
    n'est renseignée. S'applique que le changement soit normal ou anticipé (cas exceptionnel)."""
    df = df.copy()
    for t in FIXED_CYCLE_EPI:
        ...
        for i in df.index:
            ...
            try:
                ...
                next_d = d + pd.DateOffset(months=6)
                df.at[i, col_next] = next_d.strftime("%Y-%m-%d")
            except Exception:
                pass
    return df
...
# ---------- Personnel ----------
    ...
    st.caption(
        ...
        "prochaine récupération pour chaque type d'EPI. Pour Chaussures, Combinaison ou Blouse, "
        "Masque et Casque antibruit, la date prochaine récupération est **calculée automatiquement "
        "à 6 mois** après la date de récupération que tu saisis — pas besoin de la calculer toi-même. "
        "Pour un changement anticipé (cas exceptionnel), remplis quand même la nouvelle date de "
        "récupération : la prochaine échéance sera recalculée à partir de cette nouvelle date, et "
        "coche la case « Cas exceptionnel » pour garder une trace du motif dans l'archive. "
        "Gants et Lunettes n'ont pas de cycle fixe : remplis-les seulement quand ils sont changés. "
        "La case « Confirmé » sert à valider que le changement a bien été fait. "
        "Dates au format AAAA-MM-JJ (ex. 2026-07-17)."
    )
