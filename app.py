...
CONTROL_TYPES = ["Étalonnage", "Maintenance", "Vérification"]
FREQ_OPTIONS = ["", "Quotidien", "2x/semaine", "3x/semaine", "Hebdomadaire",
                "Mensuel", "Trimestriel", "Semestriel", "Annuel"]
FREQ_DAYS = {
    "Quotidien": 1, "2x/semaine": 3.5, "3x/semaine": 2.3, "Hebdomadaire": 7,
    "Mensuel": 30, "Trimestriel": 91, "Semestriel": 182, "Annuel": 365,
}

EQUIP_BASE_COLS = ["Identifiant", "Nom / type", "Constructeur", "Numéro de série", "Criticité",
                   "Emplacement", "Date d'achat"]
EQUIP_CONTROL_COLS = []
for _ct in CONTROL_TYPES:
    EQUIP_CONTROL_COLS += [f"Fréquence {_ct}", f"Dernier contrôle {_ct}"]
EQUIP_COLS = EQUIP_BASE_COLS + EQUIP_CONTROL_COLS
...
def log_epi_changes(old_df, new_df):
    """Archive dans l'onglet Historique_EPI tout EPI dont les infos changent."""
    try:
...
def fix_date_columns(df):
    ...
    for col in df.columns:
        if "date" in col.lower() or "contrôle" in col.lower():
...
def control_status(row, control_type):
    freq_label = str(row.get(f"Fréquence {control_type}", "")).strip()
    last = row.get(f"Dernier contrôle {control_type}", "")
    if not freq_label or freq_label not in FREQ_DAYS or not last:
        return None
    try:
        last_d = pd.to_datetime(last)
    except Exception:
        return None
    due = last_d + pd.Timedelta(days=FREQ_DAYS[freq_label])
...
if page == "Tableau de bord":
    ...
    if not st.session_state.equip.empty:
        for ct in CONTROL_TYPES:
            ...
            eq[f"Statut {ct}"] = eq.apply(lambda r: control_status(r, ct), axis=1)
            alertes = eq[eq[f"Statut {ct}"].astype(str).str.contains("🔴|🟠", na=False)]
            st.subheader(f"Alertes {ct.lower()}")
            if alertes.empty:
                st.success(f"Aucune alerte {ct.lower()} en cours.")
            else:
                st.dataframe(alertes[["Identifiant", "Nom / type", f"Statut {ct}"]], use_container_width=True)
...
# ---------- Personnel ----------
    ...
    with tab_vue:
        ...
        if st.button("💾 Sauvegarder Personnel", type="primary"):
            ...
            if nb_archives:
                st.success(f"Sauvegardé. {nb_archives} ancien(s) EPI archivé(s).")
            else:
    ...
    with tab_form:
        ...
        with st.form("form_personnel"):
            ...
            st.markdown("**EPI sur demande (pas de cycle fixe)**")
            valeurs_demande = {}
...
# ---------- Équipements ----------
    ...
    st.caption("Fréquences disponibles : " + ", ".join([f for f in FREQ_OPTIONS if f]))
    ...
    with tab_form:
        ids_existants = ["-- Nouvel équipement --"] + [
            f"{i} — {row.get('Identifiant', '')} ({row.get('Nom / type', '')})"
            for i, row in st.session_state.equip.iterrows()
        ]
        choix = st.selectbox("Choisir un équipement à modifier, ou créer un nouveau", ids_existants)
        idx_edit = None
        ...
        with st.form("form_equip"):
            ...
            nom_type = st.text_input("Nom / type", value=row_data.get("Nom / type", ""))
            ...
            st.markdown("**Contrôles (étalonnage, maintenance, vérification)**")
            valeurs_controle = {}
            for ct in CONTROL_TYPES:
                c1, c2 = st.columns(2)
                freq_actuelle = row_data.get(f"Fréquence {ct}", "")
                idx_freq = FREQ_OPTIONS.index(freq_actuelle) if freq_actuelle in FREQ_OPTIONS else 0
                freq = c1.selectbox(f"Fréquence {ct}", FREQ_OPTIONS, index=idx_freq, key=f"eq_freq_{ct}")
                dernier = c2.text_input(f"Dernier contrôle {ct} (AAAA-MM-JJ)", value=row_data.get(f"Dernier contrôle {ct}", ""), key=f"eq_dernier_{ct}")
                valeurs_controle[ct] = (freq, dernier)

            col_save, col_delete = st.columns(2)
            submitted = col_save.form_submit_button("💾 Enregistrer", type="primary")
            deleted = col_delete.form_submit_button("🗑️ Supprimer cet équipement") if idx_edit is not None else False

        if submitted:
            new_row = {
                "Identifiant": identifiant, "Nom / type": nom_type, "Constructeur": constructeur,
                "Numéro de série": num_serie, "Criticité": criticite, "Emplacement": emplacement,
                "Date d'achat": date_achat,
            }
            for ct, (freq, dernier) in valeurs_controle.items():
                new_row[f"Fréquence {ct}"] = freq
                new_row[f"Dernier contrôle {ct}"] = dernier
    ...
    with tab_couleur:
        if st.session_state.equip.empty:
            ...
            aff = st.session_state.equip[["Identifiant", "Nom / type"]].copy()
            for ct in CONTROL_TYPES:
                aff[ct] = st.session_state.equip.apply(lambda r: control_status(r, ct) or "⚪", axis=1)
            styled = aff.style.map(style_status_cell, subset=CONTROL_TYPES)
            st.dataframe(styled, use_container_width=True)
...
# ---------- Import depuis l'Excel existant ----------
    ...
    st.caption(
        ...
        "Les dates sont converties automatiquement."
    )
